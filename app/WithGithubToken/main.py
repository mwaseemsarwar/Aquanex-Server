...
from .llm_service import stream_openai, stream_fallback, stream_github
...
async def chat(payload: ChatRequest):
    try:
        ...
        selected_model = payload.selectedModel or settings.OPENAI_MODEL
        ...
        r = await get_redis()
        cache_key = cache_key_from_prompt(latest, selected_model)

        async def stream_result(gen):
            """Helper to stream and cache results."""
            full = []
            try:
        ...
        if not (is_informal or is_allowed):
            return StreamingResponse(
                stream_result(stream_fallback()), media_type="text/plain; charset=utf-8"
            )
            ...
            try:
                ...
                if cached:
                    async def stream_cached():
                        for ch in cached:  # cached is str already
                            yield ch
                            await asyncio.sleep(0)
                    return StreamingResponse(
                        stream_cached(), media_type="text/plain; charset=utf-8"
                    )
            except Exception as e:
                logger.warning("redis_get_failed", error=str(e))

        # --- Model Routing ---
        if selected_model and selected_model.startswith("github/"):
            return StreamingResponse(
                stream_result(stream_github(messages, selected_model.replace("github/", ""))),
